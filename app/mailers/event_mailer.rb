class EventMailer < ApplicationMailer
  ...
  def first_reminder(user, invite)
    reminder_helper(user, invite, "First reminder about Local Welcome #{invite.event.chapter.name}'s #{invite.event.title}")
  end

  def last_reminder(user, invite)
    reminder_helper(user, invite, "Final reminder about Local Welcome #{invite.event.chapter.name}'s #{invite.event.title}")
  end
